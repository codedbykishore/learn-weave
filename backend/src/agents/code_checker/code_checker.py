...
class ESLintValidator:
    ...
    def __init__(self, eslint_base_dir=None):
        ...
        if eslint_base_dir:
            ...
            # Check common locations including frontend
            backend_dir = os.path.dirname(os.path.dirname(os.path.dirname(self.script_dir)))
            ...
            if not self.eslint_base_dir:
                print(f"WARNING: Could not find ESLint setup in any of: {possible_locations}")
                print("ESLint validation will be disabled. Install frontend dependencies to enable it.")
                self.eslint_base_dir = None
                return
        ...
        # Use .cmd on Windows, regular executable on Linux/Mac
        import platform
        if platform.system() == 'Windows':
            self.eslint_executable = os.path.join(self.node_modules_path, '.bin', 'eslint.cmd')
        else:
